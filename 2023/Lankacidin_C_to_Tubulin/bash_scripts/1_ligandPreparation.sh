...
cp $neededFiles/LIGleap.in .
tleap -f LIGleap.in

mkdir LIG.acpype
cp LIG_AC.* LIG.acpype
$ACPYPE/acpype.py -i LIG.mol2 -b LIG
...
cp ../gdp.pdb .
cp $neededFiles/gdp* .
cp $neededFiles/frcmod.phos .
cp $neededFiles/leaprc.ff99SBildn .

tleap -f gdpleap.in

mkdir gdp.acpype
cp gdp_AC.* gdp.acpype
$ACPYPE/acpype.py -i gdp.pdb -b gdp -n -3
