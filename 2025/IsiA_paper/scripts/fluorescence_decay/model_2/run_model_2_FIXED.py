...
# FIXED: Remove the problematic NeededFiles copy operations
# The required files (construct_monomer_with_CT.py and extended_most_occ_pH7_CT.pdb)
# are already in the same directory as this script

script_dir = Path(__file__).resolve().parent
print(f"Script directory: {script_dir}")
print(f"Files in directory: {list(script_dir.glob('*.py'))}")
...
def run_simulation(time_fl, time_isc, coupling_value, seed=1, config_name='ct_model'):
    ...
    # FIXED: Comment out the problematic copy commands that reference non-existent NeededFiles
    # os.system(f'cp -r ../NeededFiles/run_analysis.py {run_dir}')
    # os.system(f'cp -r run_model2.py {run_dir}')
    # os.system(f'cp -r unified_parameters.py {run_dir}')
