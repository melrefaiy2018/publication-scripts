...
# Use resolved parents to reach the project `fl_decay` root, then NeededFiles
"""
...
print(f"\n{'='*60}")
print("RATIO GENERATION STATUS:")
print(f"{'='*60}")
print(f"Available components: {[c for c, a in available.items() if a]}")
...
# if available.get('monomer') or available.get('ct') or available.get('ct_rp1'):
#     # If monomer or CT available, create monomer/CT ratios (0-100% monomer in steps of 10%)
#     for mon_percent in range(0, 101, 10):
#         ct_percent = 0  # model 2
#         rp1_ct_percent = 0 # model 4
#         rp1_percent = 100 - mon_percent # model 3
...
#     print(f"✓ Generated {len(ratios)} ratio combinations for monomer/CT/RP1+CT mixtures")
# else:
#     print(f"✗ No monomer, CT, or ct_rp1 components available to generate ratios")
...
if ratios:
    ...
    print("ERROR: No available components to analyze. Check your data files.")
    print(f"  Expected ensemble files in: Data/Ensemble/")
    print(f"  Expected files: FL_decay_monomer_ensemble.npz, FL_decay_monomer_ct_ensemble.npz, or FL_decay_monomer_ct_RP1_ensemble.npz")
