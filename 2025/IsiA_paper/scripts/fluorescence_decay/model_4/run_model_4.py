...
def construct_ratematrix_with_ct_rp1(protein_atomic, seed, run_dir, time_fl, time_isc, time_to_ct, time_to_rp1):
    """
    ...
    nonlinear_isia.add_rp1_to_ct(ct_pigment_name='Isia_Z_CLA_600', 
                                k_ct=1/(time_to_ct*10**6), # forward rate (CT → RP1)
    ...
    sparse.save_npz(os.path.join(rate_matrix_dir, f'K_ij_isia_monomer_CT_RP1_{seed}.npz'), nonlinear_isia.k_ij)
...
def generate_parameter_combinations():
    ...
    config_name = os.environ.get('ENSEMBLE_CONFIG', 'model_4')
    param_set = get_parameter_set(config_name)
    
    param_keys = list(param_set.keys())
    param_values = [param_set[key] for key in param_keys]
...
def run_simulation(time_fl, time_isc, time_to_ct, time_to_rp1, coupling_value, seed=1):
    """
    ...
    run_dir = f'Simulation/Run_FL{time_fl}_ISC{time_isc}_CT{time_to_ct}_RP{time_to_rp1}_coupling_{coupling_value}_CT_energy_{CT_ENERGY}'
    os.makedirs(run_dir, exist_ok=True)
    
    # copy the needed files to the run directory (ignore errors if file already exists)
    os.system(f'cp -r ../NeededFiles/run_analysis.py {run_dir} 2>/dev/null || true')
    os.system(f'cp -r run_model_4.py {run_dir} 2>/dev/null || true')

    # Setup logging
    logger = setup_logging(run_dir)
    logger.info(f"Starting analysis with seed = {seed}")
    logger.info(f"Run directory: {run_dir}")
    logger.info(f"Parameters: FL={time_fl}, ISC={time_isc}, CT={time_to_ct}, RP1={time_to_rp1}, Coupling={coupling_value}")
    ...
    isia_monomer_ct_rp1 = construct_monomer_with_CT(
    ...
    nonlinear_isia_with_ct_rp1 = construct_ratematrix_with_ct_rp1(
        isia_monomer_ct_rp1, seed, run_dir, time_fl, time_isc, time_to_ct, time_to_rp1
    )

    # Calculate initial probability distributions
    logger.info('Calculating initial probability distributions')
    prob_i0_ct_rp1 = np.array([np.sum(['A_CLA' in name for name in domain.list_names]) 
                           for domain in nonlinear_isia_with_ct_rp1.list_domain])
    prob_i0_ct_rp1 = prob_i0_ct_rp1 / np.sum(prob_i0_ct_rp1)

    # Calculate fluorescence
    calculate_fluorescence(nonlinear_isia_with_ct_rp1, prob_i0_ct_rp1, run_dir, seed, suffix="_ct_RP1")
