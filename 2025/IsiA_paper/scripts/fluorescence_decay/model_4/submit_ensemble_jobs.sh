#!/bin/bash
# bash submit_ensemble_jobs.sh --config model_4 --ensemble-size 20
# SLURM-based Ensemble Run for IsiA Model 4
...
echo "╔════════════════════════════════════════════════════════════╗"
echo "║  IsiA Model4 - SLURM Ensemble Submission                  ║"
...
while [ $# -gt 0 ]; do
    case $1 in
        ...
        --help)
            ...
            echo ""
            echo "Options:"
            echo "  --config CONFIG          Configuration (default: model_4)"
...
for file in run_model_4.py extended_most_occ_pH7_CT.pdb unified_parameters.py construct_monomer_with_CT.py; do
    if [ ! -f "$file" ]; then
        echo "  ✗ $file NOT FOUND"
        exit 1
    else
...
# System information
echo "╔════════════════════════════════════════════════════════════╗"
echo "║          IsiA Model 4 - SLURM Ensemble Run                 ║"
echo "╚════════════════════════════════════════════════════════════╝"
echo ""
