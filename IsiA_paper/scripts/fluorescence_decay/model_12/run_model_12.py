...
def parse_args():
    ...
    parser.add_argument('--config', type=str, default='model_12',
...
def setup_logging(run_dir, seed):
    ...
    try:
        ...
        script_name = "run_model_12"  # Fallback name
...
def construct_ratematrix_with_ct(protein_atomic, seed, run_dir, time_fl, time_isc):
    """
    Construct the rate matrix with CT state for the given protein_atomic object.
    ...
    nonlinear_isia.add_fluorescence(1 / (time_fl * 10 ** 6))
    nonlinear_isia.add_isc(1 / (time_isc * 10 ** 6))
    # Note: No CT/RP1 transitions added in model_12 - this is the key difference from other models
    nonlinear_isia._correct_diagonal()
    logger.info(f'Saving the rate matrix (with CT) with seed: {seed}')

    # Create the rate matrix directory inside the run directory
    rate_matrix_dir = os.path.join(run_dir, 'Rate_matrix')
    os.makedirs(rate_matrix_dir, exist_ok=True)

    sparse.save_npz(os.path.join(rate_matrix_dir, f'K_ij_isia_monomer_CT_{seed}.npz'), nonlinear_isia.k_ij)
    return nonlinear_isia
...
def generate_parameter_combinations(config_name='model_12'):
...
def run_simulation(time_fl, time_isc, coupling_value, seed=1, config_name='model_12'):
    """
    ...
    run_dir = f'Simulation/Run_FL{time_fl}_ISC{time_isc}_coupling_{coupling_value}_CTenergy_{CT_ENERGY}_{config_name}'
    ...
    # Updated log message to include units and CT Energy, matching the directory name
    logger.info(f"Parameters: FL={time_fl} us, ISC={time_isc} us, Coupling={coupling_value} cm-1, CT_Energy={CT_ENERGY} cm-1")
    ...
    run_params_to_save = {
        ...
        'fixed_parameters': {
             ...
             'ct_pigment': '502'        # Parameter for construct_monomer_with_CT
        },
    ...
    isia_monomer_ct = construct_monomer_with_CT(
    ...
    nonlinear_isia_with_ct = construct_ratematrix_with_ct(
        isia_monomer_ct, seed, run_dir, time_fl, time_isc,
    )
    ...
    if hasattr(nonlinear_isia, 'list_domain') and nonlinear_isia.list_domain:
        try:
            ...
            if total_prob > 0:
                ...
                logger.warning("Sum of initial probabilities (no CT) is zero. Check 'A_CLA' names in list_domain.")
    ...
    prob_i0_ct = None
    if hasattr(nonlinear_isia_with_ct, 'list_domain') and nonlinear_isia_with_ct.list_domain:
         try:
            sums_ct = [np.sum(['A_CLA' in name for name in domain.list_names])
                       for domain in nonlinear_isia_with_ct.list_domain]
            prob_i0_ct = np.array(sums_ct, dtype=float)
            total_prob_ct = np.sum(prob_i0_ct)
            if total_prob_ct > 0:
                prob_i0_ct = prob_i0_ct / total_prob_ct
            else:
                logger.warning("Sum of initial probabilities (with CT) is zero. Check 'A_CLA' names in list_domain.")
                prob_i0_ct = None  # Indicate invalid probability
         except (AttributeError, TypeError) as e:
            logger.error(f"Error accessing list_names for prob_i0_ct calculation: {e}")
            prob_i0_ct = None
    ...
    if prob_i0 is not None:
        ...
        logger.error("Skipping fluorescence calculation (no CT) due to invalid initial probability.")

    if prob_i0_ct is not None:
        calculate_fluorescence(nonlinear_isia_with_ct, prob_i0_ct, run_dir, seed, suffix="_ct")
    else:
        logger.error("Skipping fluorescence calculation (with CT) due to invalid initial probability.")
