#!/bin/bash
# bash submit_ensemble_jobs.sh --config model_12 --ensemble-size 20
# SLURM-based Ensemble Run for IsiA Model 12
...
echo "╔════════════════════════════════════════════════════════════╗"
echo "║  IsiA Model12 - SLURM Ensemble Submission                  ║"
...
while [ $# -gt 0 ]; do
    case $1 in
        ...
        --help)
            ...
            echo ""
            echo "Options:"
            echo "  --config CONFIG          Configuration (default: model_12)"
...
for file in run_model_12.py extended_most_occ_pH7_CT.pdb unified_parameters.py construct_monomer_with_CT.py; do
    if [ ! -f "$file" ]; then
        echo "  ✗ $file NOT FOUND"
        exit 1
    else
...
#SBATCH -J ensemble_@CONFIG@_@ENSEMBLE_SIZE@  # Job name
...
# System information
echo "╔════════════════════════════════════════════════════════════╗"
echo "║          IsiA Model 12 - SLURM Ensemble Run                ║"
echo "╚══════════════════════════════════════════════════════======╝"
echo ""
...
source ~/miniconda3/bin/activate isia_model
...
if [ @NUM_NODES@ -eq 1 ]; then
    # Single node - use run_model12_slurm_parallel.py
    python3 run_model12_slurm_parallel.py 0
else
    # Multiple nodes - use srun to distribute
    srun bash -c '
        NODE_ID=$((SLURM_PROCID))
        echo "Node $NODE_ID starting on $HOSTNAME"
        python3 run_model12_slurm_parallel.py $NODE_ID
