...
def parse_args():
    ...
    parser.add_argument('--config', type=str, default='model_123',
...
def generate_parameter_combinations(config_name='model_123'):
...
def run_simulation(time_fl, time_isc, time_to_rp1, coupling_ct=None, coupling_rp1=None, coupling_value=None, seed=1):
    """
    ...
    time_to_rp1 : float
        Time constant for transition to RP1 state
    coupling_ct : int, optional
        Coupling value for CT state
    coupling_rp1 : int, optional
        Coupling value for RP1 state
    coupling_value : int, optional
        Coupling value (for backward compatibility with single coupling models)
    seed : int
        Random seed for the ensemble
    """
    # Handle backward compatibility: use coupling_value if separate coupling params not provided
    if coupling_ct is None:
        coupling_ct = coupling_value
    if coupling_rp1 is None:
        coupling_rp1 = coupling_value if coupling_value is not None else coupling_ct
    
    # Create a main run directory that includes all parameters in the name
    run_dir = f'Simulation/Run_FL{time_fl}_ISC{time_isc}_RP{time_to_rp1}_coupling_CT_{coupling_ct}_RP1_{coupling_rp1}_CT_energy_{CT_ENERGY}'
    ...
    logger.info(f"Parameters: FL={time_fl}, ISC={time_isc}, RP1={time_to_rp1}, Coupling_CT={coupling_ct}, Coupling_RP1={coupling_rp1}")
    ...
    used_parameters = {
        ...
        'COUPLING_CT': coupling_ct,
        'COUPLING_RP1': coupling_rp1
    }
    ...
    run_params_to_save = {
        'run_specific_parameters': {
            ...
            'coupling_ct_cm-1': coupling_ct,
            'coupling_rp1_cm-1': coupling_rp1,
        },
        'fixed_parameters': {
    ...
    isia_monomer_rp1 = construct_monomer_with_CT(
        ...
        coupling_ct=coupling_rp1, ct_pigment='502', seed=seed
    )

    isia_monomer_ct = construct_monomer_with_CT(
        pdb_file, dir_path=run_dir, e0_a=14900,ct_energy=CT_ENERGY, e0_b=0, N_ens=100, 
        temp=300, dielectric_tresp=1, dielectric_dipole=1.5, dielectric_cdc=2, 
        coupling_ct=coupling_ct, ct_pigment='502', seed=seed
    )
    ...
    prob_i0_ct = np.array([np.sum(['A_CLA' in name for name in domain.list_names]) 
                           for domain in nonlinear_isia_with_ct.list_domain])
    prob_i0_ct = prob_i0_ct / np.sum(prob_i0_ct)
...
def main():
    ...
    if args.list_params:
        print(f"\nTotal parameter combinations for {config_name}: {total_combinations}\n")
    ...
    if args.param_index is not None:
        ...
        if "COUPLING_value" in param_dict:
            run_simulation(
                param_dict["TIME_FL"],
                param_dict["TIME_ISC"],
            ...
            run_simulation(
                ...
                param_dict["TIME_ISC"],
                param_dict["TIME_to_RP1"],
                coupling_ct=param_dict["COUPLING_CT"],
                coupling_rp1=param_dict["COUPLING_RP1"],
                seed=seed
            )
    else:
