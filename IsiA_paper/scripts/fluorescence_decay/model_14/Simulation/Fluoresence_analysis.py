...
class FluorescenceData:
    ...
    def save_simulation_data(self, fl_time_mixture, fl_wave_mixture, label):
        ...
        monomer_match = re.search(r'(\d+)%\s*M', label)
        ct_match = re.search(r'(\d+)%\s*CT', label)
        rp1_ct_match = re.search(r'(\d+)%\s*RP1\+CT', label)
        rp1_match = re.search(r'(\d+)%\s*RP1(?!\+)', label)  # RP1 not followed by +
        ...
        # Generate a unique filename that explicitly includes the monomer percentage 
        # to prevent overwriting when multiple mixtures are processed
        # filename = f"Monomer_{monomer_pct}pct_{monomer_pct}pct_M_{ct_pct}pct_CT"
