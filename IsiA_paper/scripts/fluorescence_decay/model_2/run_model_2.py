...
# Get the current working directory and point to the correct NeededFiles location
current_dir = Path(__file__).resolve().parent.parent.parent / 'NeededFiles'
...
def parse_args():
    ...
    parser.add_argument('--config', type=str, default='ct_model',
...
def generate_parameter_combinations(config_name='ct_model'):
...
def run_simulation(time_fl, time_isc, coupling_value, seed=1, config_name='ct_model'):
    """
    ...
    run_dir = f'Simulation/Run_FL{time_fl}_ISC{time_isc}_coupling_{coupling_value}_CT_energy_{CT_ENERGY}_{config_name}'
    os.makedirs(run_dir, exist_ok=True)
    
    # copy the needed files to the run directory
    # os.system(f'cp -r Analysis_class.py {run_dir}')
    # os.system(f'cp -r ../NeededFiles/class_save_ensamble.py {run_dir}')
    # os.system(f'cp -r ../NeededFiles/Fluoresence_analysis.py {run_dir}')
    os.system(f'cp -r ../NeededFiles/run_analysis.py {run_dir}')
    os.system(f'cp -r run_model2.py {run_dir}')
    os.system(f'cp -r unified_parameters.py {run_dir}')  # Copy the parameter file too
    ...
    logger.info(f"Parameters: FL={time_fl}, ISC={time_isc}, Coupling={coupling_value}")
    ...
    isia_monomer_ct = construct_monomer_with_CT(
        ...
        coupling_ct=coupling_value, ct_pigment='504'
    )
...
def main():
    ...
    if args.param_index is not None:
        ...
        print(f"Running all {total_combinations} parameter combinations for config '{config_name}'")
