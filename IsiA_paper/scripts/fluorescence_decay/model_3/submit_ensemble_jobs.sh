...
# SLURM-based Ensemble Run for IsiA Model 2
...
echo "╔════════════════════════════════════════════════════════════╗"
echo "║  IsiA Model3 - SLURM Ensemble Submission                  ║"
...
for file in run_model_3.py extended_most_occ_pH7_CT.pdb unified_parameters.py construct_monomer_with_CT.py; do
    if [ ! -f "$file" ]; then
        echo "  ✗ $file NOT FOUND"
        exit 1
    else
...
#SBATCH -J isia_ensemble           # Job name
