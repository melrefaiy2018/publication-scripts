...
For single node: python3 run_model4_slurm_parallel.py 0
For multi-node: srun python3 run_model4_slurm_parallel.py $SLURM_PROCID
...
def task(seed):
    ...
    os.environ['ENSEMBLE_CONFIG'] = config_name
    
    # Run the simulation (config is read from environment by unified_parameters)
    cmd = f"python3 {script_name} --seed {seed}"
...
if __name__ == '__main__':
    print("\n" + "="*70)
    print("IsiA Model 4 - SLURM Parallel Ensemble Runner")
    print("="*70)
    ...
    # Create process pool with SLURM-allocated CPUs (respect CPU affinity)
    num_cores = int(os.environ.get('SLURM_CPUS_PER_TASK', '96'))
